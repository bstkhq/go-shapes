...
func (r *Renderer) ApplyBlurVogel(target, mask *ebiten.Image, ox, oy, radius, colorMix float32, numSamples int, downscaling Downscaling) {
	if mask == nil {
	...
	r.refreshVogelPoints(numSamples, float64(radius))
	df := downscaling.Factor()
	if df == 1 {
		r.applyBlurVogelDirect(target, mask, ox, oy, radius, colorMix, numSamples)
	} else {
		r.applyBlurVogelDownscaled(target, mask, ox, oy, radius, colorMix, numSamples, df)
	}
}

// helper function for ApplyBlurVogel. precondition: all input parameters have already been validated
func (r *Renderer) applyBlurVogelDirect(target, mask *ebiten.Image, ox, oy, radius, colorMix float32, numSamples int) {
	...
	r.setFlatCustomVAs01(radiusF32, colorMix)
...
func (r *Renderer) applyBlurVogelDownscaled(target, mask *ebiten.Image, ox, oy, radius, colorMix float32, numSamples int, downscale int) {
	...
	r.applyBlurVogelDirect(mid, tmp, shift, shift, radius, colorMix, numSamples)
...
func (r *Renderer) refreshVogelPoints(n int, radius float64) {
	const GoldenAngle = 2.39996322972865332223155550663361385312499901105811504293511275073130733823943 // https://oeis.org/A131988
